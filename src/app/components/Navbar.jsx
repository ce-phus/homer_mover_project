                    ...
                    {navLinks.map((link, index) => (
                        <div key={index} className="relative"
                            ...
                            <Link href={link.href} className='relative group'>
                                <p className={`font-medium text-lg tracking-wide ${active === link.title ? "text-primary" : "text-dark"} hover:text-gray-700 duration-300 ease-in-out hover:scale-105`}
                            ...
                            {link.dropdownItems && link.dropdownItems.length > 0 && (
                                <div className={`absolute -left-10 mt-2 text-center max-h-[800px] bg-white shadow-xl shadow-black w-[300px] text-black rounded-xl shadow-lg transition-opacity duration-500 z-[1000] sm:z-[1000] ${hoveredIndex === index ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
